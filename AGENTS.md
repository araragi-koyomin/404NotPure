...
`-SkipClean` 对应精确的 `mvn test`；不加该参数对应 `mvn clean test`。脚本读取被 Git 忽略的 `.env` 连接本机 Docker 中的 MySQL/Redis，但会覆盖外部服务参数并关闭真实 OSS/支付宝探针，不得修改脚本使默认测试访问真实外部服务。1 GiB 只是当前 310 项测试在固定本机环境中预先选择并验证的最低候选档位，不是理论最小内存，也不是未来测试集的永久保证。
...
8. 库存调整接口不能制造负库存，也不能把可用库存设置为与已冻结业务含义冲突的值。V3 已通过 `uk_stockpile_product_id` 和 `fk_stockpile_product` 让数据库保证每条库存属于真实商品、同一商品最多一条库存；V4 只为历史缺失商品补 `amount=0`、`frozen=0`。管理员调整接口仍可能写入负库存，该独立缺陷由 STOCK-001 跟踪。
9. 不得在事务提交前向外部系统声明订单成功。涉及缓存失效时，应在数据库事务成功后处理，或采用能够避免缓存长期脏读的等价方案。

当前 `OrderServiceImpl.addOrder` 的库存冻结由 ORD-001 建立事务边界并使用 MySQL 条件原子更新，真实 MySQL 测试覆盖事务回滚和 16 线程争抢 1 件库存。ORD-002 已通过 PR #25 增加标准 UUID `Idempotency-Key`、请求指纹和 `(user_id, idempotency_key)` 唯一约束：同一用户同键同请求返回原订单且只冻结一次库存，同键不同请求返回 HTTP 409，锁等待超时返回 HTTP 503；前端刷新或支付表单失败后复用原结算意图。该能力有 16 线程真实 MySQL 和两个后端实例并发验收，但不能延伸为生产吞吐量、真实支付或完整多渠道支付已经解决。DB-001B 已通过 PR #19 合并：V3 从数据库层保证一个商品最多一条库存并拒绝无效商品引用，V4 为历史缺失库存的商品补一条不可售的 `0/0` 库存；完整证据见 `docs/archive/2026-08-15-inventory-schema-integrity-delivery.md` 和 `docs/archive/2026-08-16-order-checkout-idempotency-delivery.md`。

## 购物车数量与库存状态不变量

- `POST /api/cart` 和 `PATCH /api/cart/{cartItemId}` 的商品 ID、数量必须是严格 JSON 正整数；不得接受缺失值、`null`、0、负数、小数、数字字符串或布尔值。
- 添加和修改数量都必须满足 `quantity <= StockPile.amount`。这里的 `amount` 已经是可用库存，不得再次减去 `frozen`。
- 购物车校验只提供当前时点的可购买反馈，不扣减、不冻结库存，也不能替代结算时的 MySQL 条件原子更新；库存可能在加入购物车后发生变化。
- 同一用户、同一商品最多一条购物车记录。普通重复加入返回业务冲突，并发重复加入最终由 V5 的 `uk_carts_user_product` 唯一约束阻止；只允许把该命名约束转换为“商品已在购物车”的业务错误。
- V5 的 `chk_carts_quantity_positive` 保证数据库中的数量大于 0。历史非正数量或重复用户商品组合必须阻止迁移，不能自动改成 1、合并或删除。
- 购物车列表使用一次批量库存查询返回 `availableStock` 和 `AVAILABLE`、`INSUFFICIENT`、`UNAVAILABLE` 三种状态。缺失库存记录只让对应商品不可结算，不得使整个列表失败；添加或修改遇到缺失库存仍按数据异常拒绝。
- 前端必须保留库存不足条目并说明原因，禁用其结算勾选且让全选跳过；修改失败后显示服务端原因并重新加载数据库值。支付成功后的购物车清理由 PAY-002 继续处理。

CART-001 已通过 PR #21 合并，完整实现、真实 MySQL、V1～V5 迁移、并发唯一约束和项目所有者页面验收证据见 `docs/archive/2026-08-16-cart-quantity-integrity-delivery.md`。
...
- `CANCELLED`：订单所有者在 30 分钟期限前主动取消，冻结库存已经恢复为可用库存。
- `CLOSED`：订单达到或超过待支付期限后关闭，冻结库存已经恢复为可用库存。

合法状态流转为 `PENDING -> PAID`、`PENDING -> CANCELLED` 和 `PENDING -> CLOSED`。后续代码必须遵守：
...
- `POST /api/orders/{orderId}/cancel` 只允许订单所有者调用。期限前取消进入 `CANCELLED`；达到或超过期限时同一个请求进入 `CLOSED`。重复取消已经 `CANCELLED` 或 `CLOSED` 的订单幂等成功，不能再次恢复库存；`PAID` 返回业务冲突。
- 自动扫描默认每 60 秒最多选择 100 个已到期订单，并通过独立事务逐单关闭。多实例扫描、支付通知和用户取消都用 `status='PENDING'` 的数据库条件更新争夺唯一处理权；只有状态更新成功的一方可以修改库存。
- 取消或关闭必须在同一事务中按商品 ID 排序执行 `amount += quantity`、`frozen -= quantity`；任一商品恢复失败时，订单状态和此前库存更新全部回滚。
- 待支付期限默认 30 分钟，由订单 `createTime` 计算。支付宝表单的 `time_expire` 使用同一个绝对到期时刻，不能在用户重复打开页面时重新获得 30 分钟。
- 当前没有退款、支付失败处理或支付宝交易关闭调用。已经打开的支付宝页面与本地关闭之间的外部晚付款风险由 PAY-004 跟踪，不能把本地 `CLOSED` 描述成支付宝侧交易一定已经关闭。
- `Orders.createTime` 只表示创建时间，不得覆盖为支付、取消或关闭时间。V2 提供独立 `paidTime` 和唯一 `alipayTradeNo`；V6 提供独立 `cancelledTime`、`closedTime` 和区分大小写的合法状态检查约束。数据库排序规则默认不区分大小写，因此约束必须显式拒绝 `pending`、`Paid` 等 Java 枚举不能识别的变体。三个时间作为向后兼容响应字段，支付宝交易号不在订单 VO 中公开。

ORD-003 已通过 PR #23 合并到个人 `master`；完整实现、V6 迁移、真实 MySQL 并发与事务测试证据见 `docs/archive/2026-08-16-order-cancellation-expiration-delivery.md`。该结论只覆盖本地订单状态和库存恢复，不代表支付宝侧交易已经关闭。
...
- 购物车改造最低覆盖：严格 JSON 正整数、普通与并发重复加入、超过库存后数据库不变、V5 异常历史数据拒绝、批量库存三状态、缺失库存和前端不可结算行为。
...
- 下单事务边界、正数校验、重复商品汇总和条件原子更新已由 ORD-001 实现；ORD-002 已通过 PR #25 增加跨后端进程的结算请求幂等和前端失败重试状态。幂等键只合并同一用户的同一次结算意图，不代替库存条件更新，也不处理支付成功后的购物车清理、支付宝侧关单或真实资金验收。DB-001B 已通过 PR #19 用 V3/V4 增加库存商品唯一约束、商品外键和历史缺失库存补齐；历史重复库存或无效商品引用会阻止迁移，不能擅自合并、删除或改写数量。
...
- 订单状态在实体中仍保存为字符串，但服务通过 `OrderStatus` 枚举集中使用四个状态，V6 通过数据库检查约束拒绝其他值。取消和超时关闭已经恢复冻结库存；退款、支付失败和支付宝交易关闭仍未实现。
...
- DB-001A 已通过 PR #3 建立 Flyway V1 完整基线和 V2 订单支付字段，并将 JPA 切换为 `ddl-auto: validate`；DB-001B 已通过 PR #19 新增 V3 库存唯一约束/商品外键和 V4 缺失库存补齐；CART-001 已通过 PR #21 新增 V5 购物车正数检查和用户商品唯一约束；ORD-003 已通过 PR #23 新增 V6 订单生命周期字段、索引和状态检查；ORD-002 已通过 PR #25 新增 V7 结算幂等字段、用户级唯一约束和字段配对检查。遗留支付实体/表清理由 DB-001C 独立跟踪。后续实体结构变化必须增加新迁移版本，不能修改任何已经应用的 V1～V7。

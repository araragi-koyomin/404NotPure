...
当前 `OrderServiceImpl.addOrder` 已通过 PR #2 合并到 `master`：它建立事务边界，并使用 MySQL 条件原子更新冻结库存；真实 MySQL 测试覆盖事务回滚和 16 线程争抢 1 件库存。该结论只适用于下单时的库存冻结，不能延伸为支付回调幂等、请求级幂等或完整订单生命周期已经解决。DB-001B 已通过 PR #19 合并：V3 从数据库层保证一个商品最多一条库存并拒绝无效商品引用，V4 为历史缺失库存的商品补一条不可售的 `0/0` 库存；完整证据见 `docs/archive/2026-08-15-inventory-schema-integrity-delivery.md`。
...
- 下单事务边界、正数校验、重复商品汇总和条件原子更新已由 ORD-001 实现并通过真实 MySQL 测试；但结算请求没有幂等键，重复提交仍可能创建不同订单，该问题由 ORD-002 跟踪。DB-001B 已通过 PR #19 用 V3/V4 增加库存商品唯一约束、商品外键和历史缺失库存补齐；历史重复库存或无效商品引用会阻止迁移，不能擅自合并、删除或改写数量。
...
- DB-001A 已通过 PR #3 建立 Flyway V1 完整基线和 V2 订单支付字段，并将 JPA 切换为 `ddl-auto: validate`；DB-001B 已通过 PR #19 新增 V3 库存唯一约束/商品外键和 V4 缺失库存补齐；CART-001 已通过 PR #21 新增 V5 购物车正数检查和用户商品唯一约束；ORD-003 已通过 PR #23 新增 V6 订单生命周期字段、索引和状态检查。遗留支付实体/表清理由 DB-001C 独立跟踪。后续实体结构变化必须增加新迁移版本，不能修改任何已经应用的 V1～V6。

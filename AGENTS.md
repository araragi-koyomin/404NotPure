...
- CACHE-001/TEST-001 已通过 PR #7 合并并归档：商品详情具备回填、短期不存在标记、随机 TTL、类型恢复、事务提交后失效、广告最新值预热和并发回填顺序，并使用 `RedisTemplate<String, Object>`、关闭无用 Redis Repository 扫描。PERF-001 的真实容器停止测量进一步证明，代码捕获 Redis 异常不等于请求能及时回退：默认客户端连接等待会阻塞商品请求，该问题由 CACHE-003 跟踪。Redis 删除失败后旧值仍可能存活到原 TTL；热门商品严格单次回填由 CACHE-002 跟踪。

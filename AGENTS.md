...
- DB-001A 已通过 PR #3 建立 Flyway V1 完整基线和 V2 订单支付字段，并将 JPA 切换为 `ddl-auto: validate`；DB-001B 已通过 PR #19 新增 V3 库存唯一约束/商品外键和 V4 缺失库存补齐，遗留支付实体/表清理由 DB-001C 独立跟踪。后续实体结构变化必须增加新迁移版本，不能修改任何已经应用的 V1～V4。

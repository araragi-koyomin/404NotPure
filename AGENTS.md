...
当前 `OrderServiceImpl.addOrder` 会批量读取商品和库存并逐项冻结，但没有事务注解或并发锁；不要把当前实现描述为已经具备防超卖能力。
...
- 唯一例外是显式执行的 `OssLifecycleProbeIT` 外部探针：其 `*IT` 命名不进入默认 Surefire 发现范围，还必须设置 `RUN_REAL_OSS_PROBE=true`；只能使用 `_validation` 隔离前缀并在 `finally` 中清理，不能作为默认单元测试运行。
...
- 下单缺少统一事务边界和并发库存控制；DTO 虽有校验注解，但结算控制器没有 `@Valid`，服务层也未防御空值、零数和负数。
...
- 测试已覆盖本轮 OSS 图片上传安全边界，但默认上下文测试仍直接依赖外部 MySQL/Redis，交易、支付和缓存均缺少行为测试；本机资源紧张时 Surefire 独立测试进程曾发生原生内存不足，早期曾使用 `-DforkCount=0` 完成 17 项测试，当前同样以不启动独立测试进程的方式完成 50 项测试。TEST-002 要求找出具体原因并恢复默认测试方式，不能把降低测试进程隔离作为永久方案。

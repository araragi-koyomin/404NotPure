...
当前 `OrderServiceImpl.addOrder` 已在 `fix/order-inventory-consistency` 中建立事务边界，并使用 MySQL 条件原子更新冻结库存；真实 MySQL 测试覆盖事务回滚和 16 线程争抢 1 件库存。该结论只适用于下单时的库存冻结，不能延伸为支付回调幂等、请求级幂等或完整订单生命周期已经解决。库存表的商品唯一约束仍由 DB-001 跟踪。

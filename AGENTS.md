...
- 测试已覆盖本轮 OSS 图片上传安全边界，但默认上下文测试仍直接依赖外部 MySQL/Redis，交易、支付和缓存均缺少行为测试；本机资源紧张时 Surefire 独立测试进程曾发生原生内存不足，早期曾使用 `-DforkCount=0` 完成 17 项测试，当前同样以不启动独立测试进程的方式完成 52 项测试。TEST-002 要求找出具体原因并恢复默认测试方式，不能把降低测试进程隔离作为永久方案。

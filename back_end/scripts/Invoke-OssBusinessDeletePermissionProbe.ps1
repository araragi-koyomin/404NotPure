...
if (-not (Test-Path -LiteralPath $EnvFile)) {
    throw "OSS permission check environment file was not found: $EnvFile"
}
...
if ($missing.Count -gt 0) {
    throw "OSS permission check is missing environment variable names: $($missing -join ', ')"
}
...
$env:RUN_REAL_OSS_PERMISSION_PROBE = 'true'

Push-Location (Join-Path $PSScriptRoot '..')
try {
    $mavenArguments = @('-Dtest=OssBusinessDeletePermissionIT')
    if ($NoFork) {

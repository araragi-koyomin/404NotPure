...
| 当前阶段 | PERF-001 已通过 [PR #11](https://github.com/araragi-koyomin/404NotPure/pull/11) 以 squash 方式合并到个人 `master`，实现和测量结果正在本次文档批次转入冷层。下一开发任务尚未创建代码分支；建议先讨论 CACHE-003，因为 PERF-001 证明 Redis 容器完全停止时，计划 50 QPS、10 秒的故障窗口内没有业务请求完成，而热点缓存失效场景虽然变慢但 100/100 请求仍返回正确结果。 |
...
| 尚未完成 | CACHE-003 尚未确定 Redis 连接与命令超时、数据库回退容量、限流和快速失败策略，也尚未创建开发分支或开始 TDD；CACHE-002 尚未实现热门商品同一时刻只由一个请求回填缓存。PERF-001 的结果只能证明固定本机环境下的相对差异，不能外推为生产容量。 |
| 当前阻塞或待确认 | PERF-001 没有合并阻塞。其正式三轮结果实际为 100% 正确、0 错误、0 丢弃，但由收紧前允许正确率高于 99%、错误和丢弃低于 1% 的脚本生成；严格脚本只完成两次 1 QPS 安全烟雾。高负载代表性复测因可用内存 7.74 GiB 低于 8 GiB 安全线而拒绝开始，且项目所有者明确禁止关闭其他内容或降低安全线。该证据限制已写入冷层，不再阻止归档；CACHE-003 的具体超时与降级方案需要在下一轮讨论确认。 |
| 下一步 | 完成本次 PERF-001 文档归档 PR；随后先讨论并确认 CACHE-003 的故障时间上限、数据库回退容量和快速失败规则，更新温层计划并创建独立开发分支。CACHE-002 保持 P1，但排在 Redis 完全停止导致请求不可用的问题之后。 |

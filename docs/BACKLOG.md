...
| 下一步 | 完成 ORD-002 冷层归档后，按依赖顺序优先讨论 API-001 订单详情/列表，再处理 PAY-002 服务端可信支付返回页；未经项目所有者确认不直接开始实现。 |
...
CART-001、ORD-003 与 ORD-002 合并归档后，优先按以下依赖顺序选择开发批次；顺序变化时必须同时更新本节和对应温层计划：

1. API-001：补齐带订单所有权校验的详情和列表读取接口；
2. PAY-002：支付同步返回页改为查询服务端订单状态；

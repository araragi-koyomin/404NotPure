...
ORD-003 默认允许用户取消自己的待支付订单，并每 60 秒扫描最多 100 个已经等待 30 分钟的订单。取消或超时关闭会在同一个数据库事务中把冻结库存恢复为可用库存。这里的“关闭”目前只改变本项目数据库；支付宝交易关闭和迟到支付处理由后续 PAY-004 跟踪。

| 变量 | 默认值 | 说明 |
|---|---:|---|
| `ORDER_EXPIRATION_ENABLED` | `true` | 是否启动自动扫描。关闭后仍保留用户主动取消和支付前到期检查。默认测试会关闭后台调度，避免测试进程修改不属于当前用例的历史订单。 |
| `ORDER_PENDING_TIMEOUT` | `30m` | 从订单 `create_time` 起计算的待支付期限。用户在期限前取消进入 `CANCELLED`，达到或超过期限后进入 `CLOSED`。支付宝支付表单使用同一个绝对到期时刻，不能每次打开页面都重新获得 30 分钟。 |
| `ORDER_EXPIRATION_SCAN_INTERVAL` | `60s` | 两次自动扫描之间的等待时间。必须是带单位的正时间，例如 `60s`。 |
| `ORDER_EXPIRATION_BATCH_SIZE` | `100` | 每轮最多处理的历史待支付订单数。每个订单使用独立事务，一个订单数据异常不会回滚同一轮已经成功关闭的其他订单。 |

这些变量不是密钥，可以保留默认值。若面试演示时暂时关闭自动扫描，应明确说明这只是诊断开关，不能把长期关闭后的冻结库存积压描述成已解决。

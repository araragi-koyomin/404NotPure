...
Surefire 是 Maven 运行 JUnit 测试的组件。它默认会启动独立的 Java 测试进程，使测试使用的内存、系统属性和退出行为与 Maven 主进程隔离。项目曾在这种独立进程中出现原生内存不足，随后使用 `-DforkCount=0` 让测试直接运行在 Maven 主进程中，完成了当前 52 项测试。这个替代方式证明了测试内容能通过，但不能永久代替正常的测试进程隔离。

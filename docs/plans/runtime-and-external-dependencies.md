...
完成标准：自动探针一次通过完整 Put→Get→Delete→404；故障路径能够执行清理；运行身份无法删除 avatar、product 或 advertisement 对象；默认单元测试不访问真实 OSS。之后 OSS-001 进入“待评审/待合并”，merge 后按三层记忆规则从 BACKLOG 移除并归档证据。
...
2026-08-09 使用轮换后的新凭据再次执行显式探针，1/1 通过完整 Put→匿名 GET 200→Delete→GET 404，验证对象没有残留。Maven 输出仅包含构建和测试摘要，没有出现 Authorization、签名、AccessKey、Endpoint、Bucket 或对象名。新构建后端随后在本机 8080 启动，商品接口直连和经 5173 Vite 代理均返回 HTTP 200，启动日志未发现 MySQL/Redis 连接失败或 OSS 敏感日志标记。OSS-001 与 SEC-002 已解除阻塞，现处于待评审/待合并；merge 后再依三层记忆规则归档并从 BACKLOG 移除。
...
2026-08-10 优先级决定：当前交付目标是本机运行和面试演示，前端、后端、MySQL、Redis 的混合运行方式已经验证可用，因此完整四容器编排从 P1 调整为长期 P2。RUN-002 仍保持 blocked，表示完整 Compose 尚未验收；优先级降低不代表问题已经解决，也不能据此宣称四容器部署可用。
...
当前阻塞：Docker Hub 对 Temurin 17 镜像请求持续超时，ECR manifest 请求被限流。不得因为本机混合模式成功而把四容器验证标记完成。

历史配置缺陷已经修复：同一份 `back_end/.env` 在本机混合模式中使用 `DB_HOST=127.0.0.1`、`DB_PORT=3307`，而 Compose 内 backend 必须使用 `DB_HOST=db`、`DB_PORT=3306`。当前 Compose 已同时显式覆盖这两个容器内值，`Test-ComposeConfiguration.ps1` 也会检查它们，避免再次继承宿主机端口。RUN-002 尚未完成的原因是镜像拉取受限，以及四个容器的健康检查、前端代理和数据卷重启行为还没有完成一次完整验收。

2026-08-10 运行状态复查发现，磁盘上的 Compose 虽已把 Redis 端口限制为 `127.0.0.1:6379:6379`，但正在运行的 Redis 容器创建于该修改之前，`docker compose ps` 仍显示实际发布地址为 `0.0.0.0:6379`。因此静态配置正确不等于运行实例已经安全更新。SEC-008 要求在保留数据卷的前提下更新 Redis 容器，并再次检查实际发布地址、Redis 连接和 Compose 配置；不得把仅通过 YAML 检查描述成运行时风险已经消除。

SEC-008 已于 2026-08-10 完成运行修复并等待独立复核：只重新创建 Redis 服务容器，没有删除命名数据卷，也没有重启 MySQL；重启前后 `DBSIZE` 都为 0。新容器恢复 healthy 后，实际发布地址为 `127.0.0.1:6379`，`redis-cli PING` 和 `Test-ComposeConfiguration.ps1` 均通过。后续每次修改端口绑定都应同时检查 Compose 解析结果和正在运行的容器发布地址。
